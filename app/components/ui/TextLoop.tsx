...
// A non-breaking space. SVG <text> collapses ordinary whitespace, which would
// eat the padding between one repetition of the phrase and the next.
...
const TextLoop = ({
  ...
  const unit = useMemo(() => {
    ...
    // Escaped, not literal: these are non-breaking spaces. SVG <text>
    // collapses ordinary whitespace, which would eat the padding between
    // one repetition and the next.
    const gap = separator ? `${NBSP}${separator}${NBSP}` : NBSP.repeat(3);
    return `${base}${gap}`;

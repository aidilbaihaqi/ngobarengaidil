...
export type Card = {
  title: string;
  text: string;
  /** Foto dari katalog Unsplash. */
  photo?: Photo;
  /** Tangkapan layar lokal di public/ — dipakai kalau ada, mengalahkan `photo`. */
  local?: { src: string; alt: string };
};
...
/** One photo in the scattered achievement wall. Placement lives in the component. */
export type Shot = { src: string; alt: string };
/** One place to put your work where other people can find it. */
export type Channel = {
  icon: "social" | "linkedin" | "web";
  effort: string;
  title: string;
  text: string;
  action: string;
...
export type Slide =
  ...
  | (SlideBase & { layout: "gallery"; kicker: string; quote: string; body: string; footer: string; shots: Shot[] })
  | (SlideBase & { layout: "channels"; kicker: string; title: string; subtitle: string; channels: Channel[]; footer: string })
